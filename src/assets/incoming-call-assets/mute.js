...
const Mute = ({muteButtonHandler}) => {
  ...
  return (
    <TouchableOpacity
      ...
      onPress={() => {
        let val = Number(!animatedValue.value);
        animatedValue.value = withTiming(val, {
          duration: 300,
          easing: Easing.elastic(0.9),
        });
        muteButtonHandler(Boolean(val));
      }}>
      <Svg viewBox="0 0 100 100">
        ...
        />
        <Path
          d={`M${rX - 8},${pathY} C27,80 73,80 ${rX + rWidth + 8},${pathY}`}
          fill={'none'}
          stroke={color}
          strokeWidth={7}
        />
